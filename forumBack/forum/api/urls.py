...
urlpatterns = [
    ...
    path('categorys', addCategory, name='Добавление новой категории, метод POST'),
    path('categorys/<int:pk>', detailCategory, name='Управления категориями, методы "GET","PATCH","DELETE"'),
    path('theme', listAndPostTheme, name='Список тем и добавление новых для авторизованных пользователей, методы "GET","POST"'),
    ...
    path('comment', listAndPostComment, name='Список комментариев и добавление новых для авторизованных  пользователей, методы "GET","POST"'),
